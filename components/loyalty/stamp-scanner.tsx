...
export function StampScanner({ businessId }: { businessId: string }) {
  ...
  useEffect(() => {
    // BarcodeDetector disponible sur Chrome Android/Desktop
      ...
      const scanFrame = async () => {
        ...
        try {
          if (videoRef.current.readyState >= 2) {
            ...
            if (codes.length > 0) {
              stopCamera();
              await lookupCard(codes[0].rawValue);
              return;
            }
          }
        } catch {
          ...
          <>
            ...
            >
              <div className="relative flex-1">
                ...
                <input
                  ref={inputRef}
                  className="w-full rounded-xl border border-slate-200 bg-white py-3 pl-10 pr-4 text-sm text-slate-800 placeholder-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
